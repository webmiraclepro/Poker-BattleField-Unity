...
namespace HoldemHand
{
    ...
    public partial class Hand : IComparable
    {
        ...
        {
          lock ((object) Hand.srand)
            return Hand.RandomHand(Hand.srand, list, dead, ncards);
        }

        public static ulong RandomHand(Random rand, ulong[] list, ulong dead, int ncards)
        {
          ulong shared;
          do
          {
            shared = list[rand.Next(list.Length - 1)];
          }
          while (((long) shared & (long) dead) != 0L);
          return Hand.RandomHand(rand, shared, dead, ncards);
        }

        public static ulong RandomHand(ulong shared, ulong dead, int ncards)
        {
          ulong num1 = shared;
          int num2 = ncards - Hand.BitCount(shared);
          lock ((object) Hand.srand)
          {
            for (int index = 0; index < num2; ++index)
            {
              ...
              do
              {
                num3 = 1UL << Hand.srand.Next(52);
              }
              while ((((long) dead | (long) num1) & (long) num3) != 0L);
              num1 |= num3;
            }
          }
          return num1 | shared;
        ...
        public static ulong RandomHand(Random rand, ulong shared, ulong dead, int ncards)
        {
          ulong num1 = shared;
          int num2 = ncards - Hand.BitCount(shared);
          for (int index = 0; index < num2; ++index)
        ...
        public static ulong RandomHand(Random rand, ulong dead, int ncards) => Hand.RandomHand(rand, 0UL, dead, ncards);

        public static ulong RandomHand(ulong dead, int ncards)
        {
          lock ((object) Hand.srand)
            return Hand.RandomHand(Hand.srand, 0UL, dead, ncards);
        }

        public static ulong RandomHand(Random rand, int ncards) => Hand.RandomHand(rand, 0UL, 0UL, ncards);
